...
//		result = uvc_find_device2(mContext, &mDevice, vid, pid, NULL, fd);
		...
		if (LIKELY(!result)) {
			...
			if (LIKELY(!result)) {
				...
				{
				//mPreview = new UVCPreviewIR(mDeviceHandle);
				LOGE("opened ir camera:ok=%d", result);
				}
				else//Common device
				{
...
int UVCCamera::release() {
	...
	if (LIKELY(mDeviceHandle)) {
		...
		// プレビューオブジェクトを破棄
		LOGE("UVCCamera::release() 0%x/n",mPreview);
		SAFE_DELETE(mPreview);
		LOGE("UVCCamera::release() 1%x/n",mPreview);
		// カメラをclose
...
int UVCCamera::isCorrect() {
	ENTER();
	int result = EXIT_FAILURE;
	if (mPreview) {
		result = mPreview->isCorrect();
	}
	RETURN(result, int);
}
